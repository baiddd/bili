...
| Pas de fichier téléchargé — recherche menée directement sur `docs.libretro.com/guides/cli-intro`, le man page `retroarch(6)`, et les issues GitHub `github.com/libretro/RetroArch/issues/12577`, `#15416`, `#15424` (citées telles quelles dans le commentaire de `EmulatorProvider::launchGame()`). | Aucune syntaxe de ligne de commande documentée/stable ne permet de pointer RetroArch directement sur un fichier à l'intérieur d'une archive `.zip` — la forme interne `archive.zip#entry.rom` que l'UI de RetroArch construit en interne échoue quand elle est fournie en argument de ligne de commande (confirmé par les issues ci-dessus). `launchGame()` extrait donc l'entrée visée vers un fichier temporaire et pointe RetroArch dessus, plutôt que de parier sur une syntaxe non garantie. | Ajouté pour: sous-projet 3, lancement de jeux depuis une archive (tâche 6) |
| Pas de fichier téléchargé — recherche menée directement sur `docs.libretro.com/guides/input-controller-drivers/` (référence officielle des drivers d'entrée de RetroArch). | Confirmation que `input_joypad_driver = "sdl2"` est une valeur réelle et documentée sur Windows (aux côtés de `dinput`/`xinput`/`hid`), et que le driver par défaut sur Windows est `xinput` — un backend différent de celui (`sdl2`) que `RetroArchAutoconfig` suppose déjà via `input_driver = "sdl2"` dans les profils qu'il génère (tâche 7). `EmulatorProvider::writePortableRetroArchConfig()` fixe donc explicitement `input_joypad_driver = "sdl2"` pour que le backend de lecture des manettes corresponde à la numérotation que les profils générés supposent. Vérification sur matériel réel (manette physique, RetroArch réel, réponse des boutons) encore à faire par l'utilisateur, conformément à la convention du projet. | Ajouté pour: sous-projet 3, autoconfiguration manette (vague de correctifs finale) |
| Pas de fichier téléchargé — recherche menée directement sur la documentation de `SDL_joystick.h` (vendored avec SDL2, citée telle quelle dans le commentaire de `EmulatorProvider::ensureGamepadAutoconfig()`). | Sécurité multi-thread de `SDL_NumJoysticks`/`SDL_IsGameController`/`SDL_GameControllerNameForIndex`/`SDL_GameControllerMappingForDeviceIndex`, appelées depuis le thread GUI (`ensureGamepadAutoconfig()`) alors que `GamepadBridge` gère ces mêmes manettes sur son propre thread de poll. Les pages wiki individuelles de ces fonctions ne documentent pas leur sécurité multi-thread, mais `SDL_joystick.h` documente exactement ce scénario via `SDL_LockJoysticks()`/`SDL_UnlockJoysticks()` : « If you are using the joystick API or handling events from multiple threads you should use these locking functions to protect access to the joysticks. [...] you are guaranteed that the joystick list won't change, so the API functions that take a joystick index will be valid, and joystick and game controller events will not be delivered [while held]. » — `ensureGamepadAutoconfig()` encadre donc ces quatre appels par `SDL_LockJoysticks()`/`SDL_UnlockJoysticks()` plutôt que de les appeler nus. | Ajouté pour: sous-projet 3, autoconfiguration manette (tâche 7) |
